...
One clarification: "doesn't argue" describes the library's rhetorical stance, not its readability. A claim-by-claim reference can still be organized to read start to finish — grouped by topic instead of dumped in claim-ID order, written in plain sentences rather than bare data fields. Neutral and pleasant to read aren't in tension; `library/sources.md` is organized by topic for exactly this reason.

The sibling relationship isn't only about that distinction, either. The library isn't meant to stay "the data behind the artifacts" indefinitely — the intent from the start was for it to grow into its own resource, closer to a small reference library than a citations file. Filing it under `/artifacts/` would have boxed it in as support material for other pieces. As a sibling, it's free to become whatever it grows into, while staying under the same roof.
